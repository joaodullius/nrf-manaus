...
LOG_MODULE_REGISTER(sensor_lux_adv, LOG_LEVEL_INF);
 
static const struct device *init_sensor(void)
 {
	 const struct device *dev = DEVICE_DT_GET_ONE(rohm_bh1750);
 ...
 static int read_sensor_data(const struct device *sensor, sensor_data_t *data) {
    ...
    // Log sensor values
    LOG_DBG("Light: %.3f lx", sensor_value_to_double(&light));

    // Populate sensor_data_t structure
    data->company_id = COMPANY_ID;
    data->type = SENSOR_TYPE_LIGHT;
    data->timestamp = timestamp;
    data->values[0] = (int16_t)(sensor_value_to_double(&light) * LIGHT_SCALING_FACTOR);
